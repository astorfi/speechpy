...
class Test_Processing(object):
    ...
    def test_stack_frames(self):
        ...
        all_frames = int(np.ceil((signal.shape[0] - window) / step))
    ...
    def test_cmvn(self):
        ...
        normalized_feature = processing.cmvn(
            feature_vector, variance_normalization=True
        )
        ...
        assert np.allclose(
            np.mean(normalized_feature, axis=0),
            np.zeros((1, normalized_feature.shape[1])),
        )
        assert np.allclose(
            np.std(normalized_feature, axis=0),
            np.ones((1, normalized_feature.shape[1])),
        )
